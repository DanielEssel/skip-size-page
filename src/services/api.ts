...
export const fetchSkips = async (postcode: string = 'NR32', area: string = 'Lowestoft'): Promise<ApiResponse> => {
  // Convert the API data to match our frontend types
  const skips = [
    ...
    size: skip.size,
    type: 'Yard',
    price: Math.round(skip.price_before_vat * (1 + skip.vat/100)), // Include VAT in price
    currency: '£',
    hirePeriod: skip.hire_period_days,
    image: `https://images.pexels.com/photos/3850512/pexels-photo-3850512.jpeg?size=${skip.size}`,
